 ...
 */
#define SYSCALL_CLOBBERS "v1", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9", "memory"
...
int ar8035_main(void)
{
    ...
    /* Diagnostic: try SIOCGIFHWADDR to verify basic ioctl works */
    {
        ...
        if (ret < 0) {
            print("[!] SIOCGIFHWADDR failed errno=");
            print_dec(-ret);
            print("\n");
        } else {
    ...
    /* SIOCGMIIPHY -- query which PHY the driver uses */
    {
        ...
        if (ret < 0) {
            print("[!] SIOCGMIIPHY failed errno=");
            print_dec(-ret);
            print("\n");
        } else {
            print("[*] SIOCGMIIPHY phy_id=");
            print_hex16(ifr.u.mii.phy_id);
            print("\n");
    ...
    print("[*] AR8035 fix: eth0 phy_addr=");
    print_dec(phy_addr);
    print("\n");
    ...
    print("  debug[0x0B] before: ");
    print_hex16(val);
    println("");
    dbg_mask(0x0B, 0x8000, 0);
    val = dbg_read(0x0B);
    print("  debug[0x0B] after:  ");
    print_hex16(val);
    println("");
    ...
    val = dbg_read(0x00);
    print("  debug[0x00] before: ");
    print_hex16(val);
    println("");
    dbg_mask(0x00, 0, 0x8000);
    val = dbg_read(0x00);
    print("  debug[0x00] after:  ");
    print_hex16(val);
    println("");
